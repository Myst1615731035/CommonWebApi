...
namespace Common.Extentions.Authorization
{
    ...
    public class PermissionHandler : AuthorizationHandler<PermissionRequirement>
    {
        ...
        private readonly IRolePermissionButtonServices _roleModulePermissionServices;
        ...
        public PermissionHandler(IAuthenticationSchemeProvider schemes, IRolePermissionButtonServices roleModulePermissionServices, IHttpContextAccessor accessor)
        {
            _accessor = accessor;
            Schemes = schemes;
        ...
        protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, PermissionRequirement requirement)
        {
            ...
            {
                requirement.Permissions = (await _roleModulePermissionServices.GetPermissionItem()).ToList();
            }
            ...
            if (httpContext != null)
            {
                ...
                if (defaultAuthenticate != null)
                {
                    ...
                    // 是否开启测试环境
                    var isTestCurrent = AppConfig.Get(new string[] { "AppSettings", "UseLoadTest" }).ObjToBool();
                    ...
                    if (result?.Principal != null || isTestCurrent)
                    {
                        ...
                        if (AuthPermission.IsUseIds4)
                        {
                            isExp = (httpContext.User.Claims.SingleOrDefault(s => s.Type == "exp")?.Value) != null && UtilConvert.StampToDateTime(httpContext.User.Claims.SingleOrDefault(s => s.Type == "exp")?.Value) >= DateTime.Now;
                        }
                        else
                        {
                            // jwt
